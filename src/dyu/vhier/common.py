...
class FileUtils:
    def __init__(self, dotf: str, paths: List[str]) -> None:
        ...
        if os.path.exists(self.FILES_F) and not os.access(self.FILES_F, os.W_OK):
            ...
            raise PermissionError(f"Cannot write to directory for {self.FILES_F} (permissions?)")
        ...
        if not self.search_dirs:
            raise ValueError("No valid search directories provided or found in VERILOG_SEARCH_PATH")
        
        # Log invalid directories
        invalid_dirs = [d for d in (paths or []) + os.getenv("VERILOG_SEARCH_PATH", "").split(":") if d and not os.path.exists(d)]
    ...
    def check_files_f_duplicates(self, content: str) -> List[str]:
        ...
        incdirs = [line.strip() for line in lines if line.strip().startswith(("-incdir", "+incdir+", "-y"))]
        unique_incdirs = list(dict.fromkeys(incdirs))  # Preserve order, remove duplicates
        if len(incdirs) > len(unique_incdirs):
            print(f"Warning: Duplicate include dir entries found in {self.FILES_F}: {incdirs}")
    ...
    def deduplicate_files_f(self) -> None:
        ...
        other_lines = [line for line in lines if not line.strip().startswith(("-incdir", "+incdir+", "-y"))]
    ...
    def _search_file_in_dirs(self, file_name: str, extensions: Union[str, List[str]] = "") -> Optional[str]:
        ...
        for root in self.search_dirs:
            for dirpath, _, files in os.walk(root, followlinks=True):
                candidates = [file_name] if not extensions else [f"{file_name}{ext}" for ext in extensions]
                for candidate in candidates:
                    if candidate in files:
                        ...
                        print(f"Found {file_name} at {full_path} but cannot read (permissions?)")
    ...
    def _modify_file_content(self, content: Optional[str], existing_content: str, mode: str) -> None:
    ...
    def prepend_to_file(self, content: Optional[str]) -> None:
        ...
        self._modify_file_content(content, self._read_file(self.FILES_F) or "", "prepend")

    def append_to_file(self, content: Optional[str]) -> None:
        """Append content to the end of files.f."""
        self._modify_file_content(content, self._read_file(self.FILES_F) or "", "append")
