...
class Vhier:
    def __init__(
        ...
        if not tool_name:
            print(
                "Error: No tool specified. Provide a tool name as an argument or set VERILOG_TOOL environment variable."
            )
        ...
        if tool_name not in valid_tools:
            print(
                f"Error: Invalid tool '{tool_name}'. Valid tools: {', '.join(valid_tools)}"
            )
            sys.exit(1)
        ...
        try:
            ...
            if tool_name == "vcs":
                ...
                tool = Verilator(dotf, paths, max_iterations)
            else:
