...
def get_object_at(canvas, pos: Tuple[int, int]) -> Optional[BaseObject]:
    ...
    logging.debug(f"get_object_at: screen_pos={pos}, world_pos={world_pos}")
    print(f"get_object_at: screen_pos={pos}, world_pos={world_pos}")
    for obj in reversed(canvas.children):
        if obj.get_bounding_box().collidepoint(world_pos):
            logging.debug(f"Found object at {world_pos}: {obj.__class__.__name__}")
            print(f"Found object at {world_pos}: {obj.__class__.__name__}")
            return obj
    return None
...
def edit_properties(canvas, obj: BaseObject):
    ...
    fields = [
        ...
        ('Color R', lambda x: setattr(obj, 'color', (int(x), obj.color[1], obj.color[2])), str(obj.color[0]), canvas.font),
        ('Color G', lambda x: setattr(obj, 'color', (obj.color[0], int(x), obj.color[2])), str(obj.color[1]), canvas.font),
        ('Color B', lambda x: setattr(obj, 'color', (obj.color[0], obj.color[1], int(x))), str(obj.color[2]), canvas.font),
        ('Description', lambda x: setattr(obj, 'description', x), obj.description, canvas.small_font)
    ]
    active_field = 0
    ...
    while canvas.modal_active:
        ...
        canvas.screen.blit(modal_surf, (canvas.screen.get_width() // 2 - 200, canvas.screen.get_height() // 2 - 150))
        pygame.display.flip()
            ...
            if event.type == QUIT or (event.type == KEYDOWN and event.key == K_ESCAPE):
                ...
                if event.key == K_RETURN:
                    try:
                        ...
                        logging.debug(f"Invalid input for {fields[active_field][0]}: {input_text}")
                        print(f"Invalid input for {fields[active_field][0]}: {input_text}")
                    ...
                    active_field = (active_field + 1) % len(fields)
                    input_text = fields[active_field][2]
            fields[active_field] = (fields[active_field][0], fields[active_field][1], input_text, fields[active_field][3])
    canvas.modal_active = False
...
def handle_event(canvas, event):
    ...
    if canvas.modal_active:
        ...
        print("Modal active, skipping event")
        return True
    logging.debug(f"Handling event: type={event.type}, details={event}")
    print(f"Handling event: type={event.type}, details={event}")
    if event.type == QUIT:
    ...
    if event.type == KEYDOWN:
        ...
        if event.key == K_ESCAPE or event.key == K_q:
            ...
            logging.debug(f"Editing properties for {canvas.selected.__class__.__name__}")
            print(f"Editing properties for {canvas.selected.__class__.__name__}")
            ...
            root = Tk()
            root.withdraw()
            path = filedialog.asksaveasfilename(defaultextension=f'.{canvas.default_format}',
                                                filetypes=[('All', '*'), ('JSON', '*.json'), ('Pickle', '*.pkl'), ('SVG', '*.svg')])
            if path:
            ...
            print("Entering create mode: Rectangle")
        elif event.key == K_a:  # Explicit check for 'a'
            canvas.create_mode = 'Arrow'
            canvas.create_start = None
            logging.debug("Entering create mode: Arrow")
            print("Entering create mode: Arrow")
    elif event.type == MOUSEBUTTONDOWN:
        logging.debug(f"Mouse button down: button={event.button}, pos={event.pos}")
        print(f"Mouse button down: button={event.button}, pos={event.pos}")
        if event.button == 1:  # Left click
            if canvas.create_mode:
                canvas.create_start = screen_to_world(canvas, event.pos)
                logging.debug(f"Mouse down in create mode: {canvas.create_mode}, start={canvas.create_start}")
                print(f"Mouse down in create mode: {canvas.create_mode}, start={canvas.create_start}")
            else:
                canvas.selected = get_object_at(canvas, event.pos)
                logging.debug(f"Selected object: {canvas.selected.__class__.__name__ if canvas.selected else None}")
                print(f"Selected object: {canvas.selected.__class__.__name__ if canvas.selected else None}")
        elif event.button == 2:  # Middle click
            canvas.pan_start = event.pos
            logging.debug(f"Starting pan at {canvas.pan_start}")
            print(f"Starting pan at {canvas.pan_start}")
        elif event.button == 4:  # Wheel up
            canvas.zoom *= canvas.config['zoom_speed']
            logging.debug(f"Zoom in: {canvas.zoom}")
            print(f"Zoom in: {canvas.zoom}")
        elif event.button == 5:  # Wheel down
            canvas.zoom /= canvas.config['zoom_speed']
            logging.debug(f"Zoom out: {canvas.zoom}")
            print(f"Zoom out: {canvas.zoom}")
    elif event.type == MOUSEBUTTONUP:
        logging.debug(f"Mouse button up: button={event.button}, pos={event.pos}")
        print(f"Mouse button up: button={event.button}, pos={event.pos}")
        if event.button == 1 and canvas.create_mode and canvas.create_start:
            end_pos = screen_to_world(canvas, event.pos)
            logging.debug(f"Mouse up: creating {canvas.create_mode}, start={canvas.create_start}, end={end_pos}")
            print(f"Mouse up: creating {canvas.create_mode}, start={canvas.create_start}, end={end_pos}")
            try:
                new_obj = SUBCLASS_MAP[canvas.create_mode](
                    start=canvas.create_start,
                    end=end_pos,
                    color=canvas.config['default_color'],
                    fill_color=(255, 255, 255) if canvas.create_mode != 'Arrow' else None,
                    label="New" if canvas.create_mode == 'Text' else ""
                )
                canvas.children.append(new_obj)
                canvas.selected = new_obj
                logging.debug(f"Created object: {new_obj.__class__.__name__}, children count={len(canvas.children)}")
                print(f"Created object: {new_obj.__class__.__name__}, children count={len(canvas.children)}")
