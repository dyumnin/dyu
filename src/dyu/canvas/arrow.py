...
class Arrow(BaseObject):
    ...
    def draw(self, surface: pygame.Surface, font: Optional[pygame.font.Font] = None,canvas=None) -> pygame.Rect:
        """
        ...
        if end_pos != start_pos:
            vec = pygame.math.Vector2(screen_end_pos[0] - screen_start_pos[0], screen_end_pos[1] - screen_start_pos[1])
            angle = vec.angle_to((1,0))
            head_len = 10
            p1 = (screen_end_pos[0] - head_len * pygame.math.Vector2(1,0).rotate(angle + 30).x,
                  screen_end_pos[1] - head_len * pygame.math.Vector2(1,0).rotate(angle + 30).y)
            p2 = (screen_end_pos[0] - head_len * pygame.math.Vector2(1,0).rotate(angle - 30).x,
                  screen_end_pos[1] - head_len * pygame.math.Vector2(1,0).rotate(angle - 30).y)
            pygame.draw.line(surface, self.color, screen_end_pos, p1, 2)
            pygame.draw.line(surface, self.color, screen_end_pos, p2, 2)
        rect = pygame.Rect(min(screen_start_pos[0], screen_end_pos[0]), min(screen_start_pos[1], screen_end_pos[1]),
                          abs(screen_end_pos[0] - screen_start_pos[0]), abs(screen_end_pos[1] - screen_start_pos[1]))
        if self.label and font:
            text = font.render(self.label, True, self.color)
            mid = ((screen_start_pos[0] + screen_end_pos[0]) // 2, (screen_start_pos[1] + screen_end_pos[1]) // 2)
