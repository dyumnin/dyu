...
def draw(canvas):
    ...
    for obj in canvas.children:
        logging.debug(
            f"Drawing object: {obj.__class__.__name__}, start={obj.start}, end={obj.end}"
        )
    ...
    if canvas.create_mode and canvas.create_start:
        ...
        logging.debug(
            f"Preview: mode={canvas.create_mode}, start={canvas.create_start}, end={mouse_pos}"
        )
    ...
    if canvas.selected:
        ...
        s_box = pygame.Rect(
            world_to_screen(canvas, box.topleft),
            (box.width * canvas.zoom, box.height * canvas.zoom),
        )
