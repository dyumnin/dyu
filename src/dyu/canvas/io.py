...
def load(canvas, path: str):
    ...
    if fmt == "pickle":
        ...
        with open(path) as f:
            data = json.load(f)
        canvas.children = [
            SUBCLASS_MAP.get(o.get("__type__", "BaseObject"), BaseObject).from_dict(
                o, SUBCLASS_MAP
            )
            for o in data["objects"]
        ]
    logging.debug(
        f"Loaded {len(canvas.children)} objects: {[obj.__class__.__name__ for obj in canvas.children]}"
    )
    ...
    for obj in canvas.children:
        if isinstance(obj, canvas.SUBCLASS_MAP["Rectangle"]):
            x, y, w_, h_ = (
                obj.start[0],
                obj.start[1],
                obj.end[0] - obj.start[0],
                obj.end[1] - obj.start[1],
            )
